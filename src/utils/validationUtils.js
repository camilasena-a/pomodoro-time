 ...
 */
export function validateNumber(value, rules, fieldName) {
 ...
 */
export function sanitizeNumber(value, rules) {
    const numValue = typeof value === 'string' ? parseFloat(value) : value;
    if (isNaN(numValue)) {
...
export function sanitizeSettings(settings) {
    ...
    const sanitized = {};
    if (settings.workDuration !== undefined) {
        sanitized.workDuration = sanitizeNumber(settings.workDuration, VALIDATION_RULES.workDuration);
    }
    if (settings.shortBreak !== undefined) {
        sanitized.shortBreak = sanitizeNumber(settings.shortBreak, VALIDATION_RULES.shortBreak);
    }
    if (settings.longBreak !== undefined) {
        sanitized.longBreak = sanitizeNumber(settings.longBreak, VALIDATION_RULES.longBreak);
    }
